...
public class SubjectTestActivity extends BaseFragmentActivity implements SubjectViewListener, OnClickListener, IOnClickListener, onClickListeners, onClickListenersForError, onClickListenersForHide,
	...
	private OnPageChangeListener mPageChangeListener = new OnPageChangeListener() {
		...
		public void onPageSelected(int item) {
			...
			if ((mDatas.get(item).getSubjectType() == 5 && type == 1) || mDatas.get(item).getSubjectType() == 9) {
	...
	private void browsePics() {
		if (!picBrowseDialog.isShowing()) {
			...
			if (testData.getSubjectType()== Constant.SUBJECT_TYPE_BILL) {
				subjectData = testData.getBillData().getSubjectData();
			}else {
				subjectData = testData.getTestGroupBillData().getTestDatas().get(0).getSubjectData();				
			}
