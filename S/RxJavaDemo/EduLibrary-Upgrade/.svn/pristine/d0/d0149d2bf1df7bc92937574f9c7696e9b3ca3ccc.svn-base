...
import com.edu.library.picselect.LocalImageLoader.ImageCallBack;
...
public class ImageGroupAdapter extends BaseAdapter {
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        if (item != null) {
            ...
            // 计数
            holder.mCountTv.setText("数量："+item.getImageCount());
            holder.mImageIv.setTag(path);
            // 加载图片
            // 利用NativeImageLoader类加载本地图片
            Bitmap bitmap = LocalImageLoader.getInstance(mContext).loadImage(path,
